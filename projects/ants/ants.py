...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def find_place_bees(place, distance= 0):
            if place.entrance.is_hive or distance + 1 > self.max_range:
                return place.bees
            else:
                if place.bees and distance >= self.min_range:
                    return place.bees
                else:
                    return find_place_bees(place.entrance, distance + 1)

        return random_bee(find_place_bees(self.place))  # REPLACE THIS LINE
        # END Problem 3 and 4
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt is like the Great Wall of ants."""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt could eat any bee which in the same place as it."""
...
class TankAnt(ContainerAnt):
    """TankAnt not only defends but also attacks."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower could be placed on Water."""
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if not gamestate.created_queen:
            gamestate.created_queen = True
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # Check scared
        def check_scared():
            if self.scared_turn and destination == self.place.entrance:
                self.scared_turn -= 1
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem Optional 2
        dict, place, distance = {}, self.place, 0
        while not place.is_hive:
            for insect in place.bees:
                dict[insect] = distance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        total_damage = self.damage - distance*self.weakened_value
        actual_damage = total_damage - self.battery_limit*self.insects_shot
