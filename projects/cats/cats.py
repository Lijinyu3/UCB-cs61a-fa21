...
def choose(paragraphs, select, k):
    ...
    if len(paragraphs) == 0:
        ...
        if select(paragraphs[0]):
            return paragraphs[0] if k == 0 else choose(paragraphs[1:], select, k - 1)
        else:
...
def about(topic):
    ...
    def choose(str):
        ...
        return any([x in str for x in topic])
...
def accuracy(typed, reference):
    ...
    if len(reference_words) == 0 and len(typed_words) == 0:
        ...
        match_count = sum(typed_words[i] == reference_words[i] 
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    words_count, min = len(typed) / 5, elapsed / 60
    return words_count / min
    # END PROBLEM 4
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    return similar_word if diff_function(similar_word, typed_word, limit) <= limit else typed_word
    # END PROBLEM 5
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    count, match = 0, 0
    while count < len(sofar) and sofar[count] == prompt[count]:
        match += 1
        count += 1
    ratio = match / len(prompt)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    words = [[word_at(match, word) for word in word_indices 
        if get_fastest(word) == player] for player in player_indices]
    return words
    # END PROBLEM 10
