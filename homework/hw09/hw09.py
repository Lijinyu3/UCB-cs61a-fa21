...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r'[CS|cs]\s?\d+[a-c|A-C]?', post))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r'\b(?:2[0-3]|(?:0?|1)[0-9]):(?:[0-5][0-9])(?:AM|PM)?\b', text)
...
def most_common_code(text):
    ...
    '123'
    """
    all_area_codes = area_codes(text)
    return max(all_area_codes, key= lambda code: all_area_codes.count(code))
